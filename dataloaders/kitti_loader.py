...
class KittiDepthValAugmented(data.Dataset):
    """A data loader for the Kitti dataset
    """
    ...
    def __getitem__(self, index):
        ...
        # Convert K into torch tensor
        K = torch.from_numpy(np.array(K).reshape(1, 4)).to(dtype=torch.float32)
        # dep_np = dep.numpy().squeeze(0)
        # dep_clear, _ = outlier_removal(dep_np)
        # dep_clear = np.expand_dims(dep_clear, 0)
        # dep_clear_torch = torch.from_numpy(dep_clear)
        ...
        # candidates = {'dep': dep, 'dep_clear':dep_clear_torch, 'gt': gt, 'rgb': rgb, 'ip': dep_ip_torch, 'K': K}
        candidates = {'dep': dep, 'gt': gt, 'rgb': rgb, 'K': K}
        items = {
            key: val
            for key, val in candidates.items() if val is not None
        }
